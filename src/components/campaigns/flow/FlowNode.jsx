...
const FlowNode = ({ type, title, subtitle, position, isGhost }) => {
    const getIcon = () => {
        switch (type) {
            case 'trigger': return <MousePointerClick className="text-pulsai-blue" />;
            case 'email': return <Mail className="text-pulsai-blue" />;
            case 'wait': return <Clock className="text-pulsai-blue" />;
            ...
            case 'sms': return <MessageCircle className="text-pulsai-blue" />;
            default: return <Mail className="text-pulsai-blue" />;
        }
    };

    const getColors = () => {
        switch (type) {
            case 'trigger': return 'bg-pulsai-blue/5 border-pulsai-blue/30';
            case 'email': return 'bg-white border-pulsai-blue/20';
            case 'condition': return 'bg-white border-gray-200';
            case 'wait': return 'bg-pulsai-blue/5 border-pulsai-blue/20';
    ...
    if (type === 'condition') {
        return (
            ...
            >
                ...
                {/* Branch Labels */}
                <div className="absolute right-[-20px] top-1/2 -translate-y-1/2 bg-pulsai-green/20 text-pulsai-green text-[10px] font-bold px-1.5 py-0.5 rounded">Oui</div>
    ...
    return (
        ...
        >
            ...
            {/* Connectors */}
            <div className="absolute top-1/2 -right-1 w-2 h-2 bg-pulsai-blue rounded-full" />
