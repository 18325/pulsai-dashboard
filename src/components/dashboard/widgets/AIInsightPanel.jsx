...
const AIInsightPanel = () => {
    return (
        <motion.div
            ...
            className="relative overflow-hidden rounded-3xl p-1 bg-gradient-to-r from-pulsai-blue via-pulsai-blue/70 to-pulsai-green shadow-2xl shadow-pulsai-blue/20"
        >
            <div className="relative bg-gray-900/90 backdrop-blur-xl rounded-[20px] p-6 text-white">
                ...
                <div className="absolute bottom-0 left-0 w-64 h-64 bg-pulsai-green/25 rounded-full blur-[100px] pointer-events-none" />
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div className="flex gap-5">
                        <div className="flex-shrink-0 w-14 h-14 bg-gradient-to-br from-white/20 to-transparent rounded-2xl flex items-center justify-center border border-white/10 shadow-inner">
                            <Sparkles className="w-7 h-7 text-pulsai-green animate-pulse" />
                        </div>
                        <div>
                    ...
                    <button className="group relative px-6 py-3 rounded-xl bg-white text-gray-900 font-bold overflow-hidden transition-all hover:scale-105 active:scale-95">
                        <div className="absolute inset-0 bg-gradient-to-r from-pulsai-blue to-pulsai-green opacity-0 group-hover:opacity-20 transition-opacity" />
                ...
                <div className="mt-6 space-y-3 relative z-10">
                    {[1, 2].map((num) => (
                        <div key={`ticket-${num}`} className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer">
                            <AlertCircle size={18} className="text-red-400" />
                            <span className="text-sm font-medium text-gray-200">Ticket #120{num} : Panne serveur signalée par Client Premium (Probabilité Churn: 85%)</span>
                        </div>
                    ))}
                </div>
            </div>
