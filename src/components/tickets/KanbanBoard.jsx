...
const Column = ({ title, count, items, color }) => (
    <div className="flex-1 min-w-[300px]">
        ...
        <div className="bg-gray-50/50 rounded-3xl p-3 min-h-[500px] border border-gray-100/50">
            {items.map(ticket => (
                <TicketCard key={ticket.id} ticket={ticket} />
            ))}

            <button className={`w-full py-3 rounded-xl border-2 border-dashed border-gray-200 text-gray-400 font-medium hover:border-${color}-300 hover:text-${color}-500 transition-colors flex items-center justify-center gap-2 mt-2`}>
                <Plus size={16} />
                Ajouter Ticket
            </button>
        </div>
